...
data <- range_read(ss = sheet_url, 
           sheet = "Retrieved", 
           col_names = TRUE) %>%
  mutate(across(everything(), ~na_if(., "NA"))) %>%
  mutate(across(everything(), ~if_else(str_starts(., "The summary does not specify"), NA_character_, .)))
...
# Let's try to answer one question
qcol <- data %>%
  names %>%
  str_subset("^Q2")
qdata <- data %>%
  filter(!is.na(.[[qcol]]))
prompt <- paste0("Here is a research question:\n\n",
                 qcol,
                 "\n\n",
                 "I want you answer this question using the results listed below, which are separated by line breaks. In crafting your response, it is important to group the results into thematically similar clusters. For any given theme, if there is conflicting evidence, it is important to mention and explain that.\n\n",
                 "Results:\n\n",paste0(qdata[[qcol]], collapse = "\n\n"))
